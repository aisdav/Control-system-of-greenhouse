...
def test_readings_stats_with_values():
    ...
    stats = reading_stats(readings, sensors, "temp")
...
def test_next_command_temp_low():
    ...
    rules = ()  # Добавляем недостающий аргument rules
    
    cmds = next_command(profile, rules, snapshot)
    
    # Проверяем результат
    assert len(cmds) == 1
    assert cmds[0]["action"] == "HEATER_ON"


def test_next_command_temp_high():
    profile = PlantProfile("p1", "Tomato", (18, 25), (50, 70), (60, 80), (400, 1000), 1500)
    snapshot = {"temp": 30}  # выше нормы
    rules = ()  # Пустой tuple для rules
